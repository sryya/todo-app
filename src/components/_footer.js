...
function Footer({ selectPage, setSelectPage, taskList, setTaskList }) {
  ...
  return (
    <div>
      <Button>
        {taskList.filter((item) => item.completed === false).length} left task
      </Button>
      <span>
      ...
      {taskList.filter((item) => item.completed === true).length > 0 ? (
        <Button onClick={removeCompTask}>Clear completed</Button>
      ) : (
        false
      )}
    </div>
